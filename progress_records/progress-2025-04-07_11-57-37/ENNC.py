...
import sys
sys.path.append(r"C:\Users\ahmet\PycharmProjects\datadriven_ESS\Libraries\Libraries\Python")
...
class ENNC:
    def __init__(self, Cn, Ts, cRate_in, SoC_in, Temp_in,
        ...
        if Temp_in:
            ...
            self.VistFnc = tf.function([self.net.get_layer(name='Iin').input, self.net.get_layer(name='SoC').input,
                                       self.net.get_layer(name='Temp').input],
                                      [self.net.get_layer(name='OutIstNet').output])

            self.VdynFnc = tf.function([self.net.get_layer(name='Iin').input, self.net.get_layer(name='SoC').input,
                                       self.net.get_layer(name='Temp').input],
                                      [self.net.get_layer(name='OutDynNet').output])

            self.VdynsFnc = tf.function([self.net.get_layer(name='Iin').input, self.net.get_layer(name='SoC').input,
                                        self.net.get_layer(name='Temp').input],
                                       [self.net.get_layer(name='OutDynNet').input])

            self.VqstFnc = tf.function([self.net.get_layer(name='SoC').input, self.net.get_layer(name='Temp').input],
                                      [self.net.get_layer(name='OutQstNet').output])

            self.RistFnc = tf.function([self.net.get_layer(name='Iin').input, self.net.get_layer(name='SoC').input,
                                       self.net.get_layer(name='Temp').input],
                                      [self.net.get_layer(name='OutRistNet').output])

            self.RdynFnc = tf.function([self.net.get_layer(name='Iin').input, self.net.get_layer(name='SoC').input,
                                       self.net.get_layer(name='Temp').input],
                                      [self.net.get_layer(name='OutRdynNet').output])

            self.TauDynFnc = tf.function([self.net.get_layer(name='Iin').input, self.net.get_layer(name='SoC').input,
                                         self.net.get_layer(name='Temp').input],
                                        [self.net.get_layer(name='OutTauDynNet').output])
        else:
            self.net = Model(inputs=[Iin, SoC], outputs=Vout)

            # Define the output functions correctly
            self.VistFnc = lambda Iin, SoC: self.net.get_layer(name='OutIstNet').output
            self.VdynFnc = lambda Iin, SoC: self.net.get_layer(name='OutDynNet').output

            self.VdynsFnc = lambda Iin, SoC: self.net.get_layer(name='OutDynNet').input


            self.VqstFnc = lambda SoC: self.net.get_layer(name='OutQstNet').output


            self.RistFnc = lambda Iin, SoC: self.net.get_layer(name='OutRistNet').output

            self.RdynFnc = lambda Iin, SoC: self.net.get_layer(name='OutRdynNet').output


            self.TauDynFnc = lambda Iin, SoC: self.net.get_layer(name='OutTauDynNet').output
